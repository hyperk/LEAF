...
#define MINIMALOUTPUT
bool separatedTriggers=false;//Assume two independent triggers, one for mPMT, one for B&L
bool produceDirectionPDF=false;//false;//true;
bool thetaOnly=true;//true;
WCSimRootGeom *geo = 0; 
...
TH2D * ChargePerPMTXdistance[nPMTtypes];
//TH2D * ChargePerPMTXdistanceNorm[nPMTtypes];
...
TH2D * TotalChargeXtoWall[nPMTtypes];
...
double Omega(double R, double r_PMT){//solid angle factor(divided by 2pi)
  double constant = 1e5;//Should be set to 1, but is here to nornalize corrected charge more or less the same as the total charge
  return constant*0.5*r_PMT*r_PMT/(R*R+r_PMT*r_PMT);
}
...
int main(int argc, char **argv){
  ...
  */
  ...
  for(int i=0;i<nPMTtypes;i++){
   ...
   ChargeProfile[i] = new TH1D(Form("ChargeProfile_pmtType%d",i),"",720,0,180);
    ...
    ChargePerPMT[i] = new TH1D(Form("ChargePerPMT_pmtType%d",i),"",500,0,50);
    ChargePerPMTXdistance[i] = new TH2D(Form("ChargePerPMTXdistance_pmtType%d",i),"",100,0,1.5*TankSize,500,0,50);
    //ChargePerPMTXdistanceNorm[i] = new TH2D(Form("ChargePerPMTXdistanceNorm_pmtType%d",i),"",100,0,1.5*TankSize,500,0,50);
    ...
    TotalChargeXdWall[i] = new TH2D(Form("TotalChargeXdWall_pmtType%d",i),"",100,0,TankSize/2,1e4,0,3e4);
    TotalChargeXtoWall[i] = new TH2D(Form("TotalChargeXtoWall_pmtType%d",i),"",150,0,1.5*TankSize,1e4,0,3e4);
  ...
  */
  ...
  for (int ev=startEvent; ev<nevent; ev++)
  {
    ...
    */    
    ...
    towall = FindToWall(particleStart,particleDir);
      ...
      for (int p=0; p< nPMTs ; p++)
	{
	  ...
	  double distance_pmt_vertex=TMath::Sqrt( pow(particleStart[0]-pmt_temp.GetPosition(0),2) + pow(particleStart[1]-pmt_temp.GetPosition(1),2) + pow(particleStart[2]-pmt_temp.GetPosition(2),2)); 
	  ChargePerPMTXdistanceNorm[pmtType]->Fill(distance_pmt_vertex,1.);
	  //std::cout<<distance_pmt_vertex<<std::endl;
	}
      */
      //std::cout<<ChargePerPMTXdistanceNorm[pmtType]->GetEntries()<<std::endl;
      ...
      for (i=0; i< nhits ; i++)
	{
	  ...
	  distance_pmt_vertex=TMath::Sqrt( pow(particleStart[0]-pmt.GetPosition(0),2) + pow(particleStart[1]-pmt.GetPosition(1),2) + pow(particleStart[2]-pmt.GetPosition(2),2));
	  ...
	  if(!plotDigitized){
	    ...
	    //ChargeProfile2DXPMT[pmtType]->Fill(pmt_number_in_mPMT,PMTpos[1],PMTpos[2],peForTube);
	      ...
	      ChargePerPMTXdistance[pmtType]->Fill(distance_pmt_vertex,peForTube);

	      totalPe += peForTube;
	      totalPecorrected += peForTube/Omega(distance_pmt_vertex,PMTradius[pmtType]);
	      totalHit ++;
	      //}
      ...
      if(verbose) cout << "Total Pe : " << totalPe << ", cherenkov = "<<totalPecorrected<<endl;
      if(!plotDigitized){
	TotalCharge[pmtType]->Fill(totalPe);
	TotalHit[pmtType]->Fill(totalHit);
	TotalChargeXdWall[pmtType]->Fill(dwall,totalPe);
	TotalChargeXtoWall[pmtType]->Fill(towall,totalPe);
      }
    }
      ...
      for (i=0; i< nhits ; i++)
	{
	  ...
	  if(plotDigitized){
		...
		//cout<<"PMT type = "<<pmtType<<", pmt number = "<<tubeNumber<<", charge = "<<peForTube<<", time = "<<time<<", after shift by trigger = "<<time+triggerTime[pmtType]<<endl;
	      ...
	      ChargePerPMT[pmtType]->Fill(peForTube);
	      ChargePerPMTXdistance[pmtType]->Fill(distance_pmt_vertex,peForTube);
	      //std::cout<<"Distance pmt to vrtex = "<<distance_pmt_vertex<<std::endl;

	      totalPe += peForTube;
	      totalPecorrected += peForTube/Omega(distance_pmt_vertex,PMTradius[pmtType]);
	      std::cout<<"PMT type = "<<pmtType<<", dist = "<<distance_pmt_vertex<<", rad = "<<PMTradius[pmtType]<<", omega = "<<Omega(distance_pmt_vertex,PMTradius[pmtType])<<endl;
	      totalHit ++;
	    }
	    //if(i%50 == 0) cout << "Hit angle wrt particle direction = " << relativeAngle << ", Phi = " << phiAngle*180/TMath::Pi() << " Z position = " << PMTpos[2] << endl;
	  }
  ...
  */
  ...
  for(int i=0;i<nPMTtypes;i++){
    ...
    TotalChargeXtoWall[i]->Write();//Fill(dWall,totalPe);

    ChargePerPMTXdistance[i]->Write();
    //ChargePerPMTXdistanceNorm[i]->Write();
     }
#else
  hvtx0->Write();
  hvtx1->Write();
