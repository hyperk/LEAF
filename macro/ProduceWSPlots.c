void ProduceWSPlots(){
  ...
  TH3D * angularResponseXPMT_hits[nFiles][nPMTtypes];TCanvas * cAngularResponsePMT_hits;
  ...
  for(int f=0;f<nFiles;f++){
	...
	for(int ibinx=1;ibinx<=nPMT;ibinx++){
      ...
      /*             
      TH2D *ChargeProfile2DXPMT_2D[nPMTtypes][nPMT];
      for(int i=1;i<2;i++){
	if(f==0){
	  cChargeProfile2DXPMT = new TCanvas("cChargeProfile2DXPMT","cChargeProfile2DXPMT");
	  //nPMT = angularResponseXPMT_hits[f][i]->GetNbinsX();
	  cChargeProfile2DXPMT->Divide( floor(TMath::Sqrt(nPMT)) , floor(TMath::Sqrt(nPMT)) );
	}
	cout<<"npmt per mPMT="<<nPMT<<endl;
	
	for(int ibinx=1;ibinx<=nPMT;ibinx++){
	  cChargeProfile2DXPMT->cd(ibinx);
	  ChargeProfile2DXPMT[f][i]->GetXaxis()->SetRange(ibinx,ibinx);
	  hProjection = (TH2D*) ChargeProfile2DXPMT[f][i]->Project3D("zy");
	  ChargeProfile2DXPMT_2D[i][ibinx-1] = (TH2D*) hProjection->Clone(Form("ChargeProfile2DXPMT_2D_pmtType%d_pmt%d",i,ibinx));
	  ChargeProfile2DXPMT_2D[i][ibinx-1]->Rebin2D(1,1);
	  ChargeProfile2DXPMT_2D[i][ibinx-1]->GetXaxis()->SetRangeUser(20,80);
	  ChargeProfile2DXPMT_2D[i][ibinx-1]->GetYaxis()->SetRangeUser(20,80);
	  ChargeProfile2DXPMT_2D[i][ibinx-1]->Draw("colz");
	}
	}*/
    }
